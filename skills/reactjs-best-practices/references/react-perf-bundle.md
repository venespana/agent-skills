# React Performance — Bundle Size Optimization (CRITICAL)

Adapted from Vercel React Best Practices. Reducing initial bundle size improves TTI and LCP. `next/dynamic` adapted to `React.lazy` + `Suspense`.
...
Use `React.lazy` + `Suspense` to code-split large components not needed on initial render.
